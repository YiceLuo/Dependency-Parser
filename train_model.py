...
def build_model(word_types, pos_types, outputs):
    ...
    model = Sequential()
    model.add(keras.layers.Embedding(word_types, 32, input_length=6,name='emb'))
    model.add(Flatten())
    model.add(keras.layers.Dense(100, activation='relu',name='den1'))
    model.add(keras.layers.Dense(10, activation='relu',name='den2'))
    model.add(keras.layers.Dense(91, activation='softmax',name='den3'))
...
#python train_model.py ./data/inputs.npy ./data/outputs.npy ./model.h5
